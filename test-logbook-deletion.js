    ...
    if (fs.existsSync(notesPath)) {
        ...
        const entryStillExists = (noteKey, newEntries, date) => {
            ...
            for (const entry of newEntries) {
                ...
                const dateMatch =
                    !date || noteKey.includes(date.replace(/-/g, ""));
                const timeMatch =
                    !entry.time ||
                    noteKey.includes(entry.time.replace(":", ""));
                const authorMatch =
                    !entry.author ||
                    noteKey.includes(
                        entry.author
        ...
        Object.keys(notesData.logbookNotes).forEach((noteKey) => {
            ...
            if (noteKey.includes(`logbook_${dateStr}_`)) {
                ...
                if (!entryStillExists(noteKey, [], testDate)) {
                    // Empty array means all entries deleted
        ...
        console.log(
            `Keys that would be deleted for ${testDate}:`,
            keysToDelete
        );
                ...
                if (fs.existsSync(notesPath)) {
                    ...
                    entryKeys.forEach((entryKey) => {
                        if (
                            ...
                            deletedCount++;
                            console.log(
                                `✅ Deleted notes for logbook entry: ${entryKey}`
                            );
                        }
                    });
                    ...
                    if (deletedCount > 0) {
                        ...
                        console.log(
                            `Total logbook notes deleted: ${deletedCount}`
                        );
                    }
